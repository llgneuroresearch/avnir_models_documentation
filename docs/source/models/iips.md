...
:align: center
```
The IPH-IVH-PHE Segmentation (IIPS) is a  tool designed to segment intraparenchymal (cerebral) hemorrhage (IPH), intraventricular hemorrhage (IVH), and perihematomal edema (PHE) from CT images using a Docker container. The model was trained using the nnU-Net default configuration, with the exception of customized density clipping and the `3D-fullres` setting, on 341 non-contrast head CTs using five-fold cross-validation. We used the [Radiological Society of North America 2019 Intracranial Hemorrhage Detection Challenge](https://www.kaggle.com/c/rsna-intracranial-hemorrhage-detection) dataset for segmentation. External testing was conducted on 183 independent baseline NCCTs with spontaneous ICH at our institution. Further details are available in our upcoming publication. 


## Prerequisites

Before using IIPS, ensure the following:
- Head CT images need to be in the `nii.gz` format. 
...
- **1**: Intracparenchymal/cerebral Hemorrhage (ICH)
